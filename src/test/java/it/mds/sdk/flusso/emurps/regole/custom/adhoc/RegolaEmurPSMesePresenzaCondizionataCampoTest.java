...
class RegolaEmurPSMesePresenzaCondizionataCampoTest {
    public static final String ASSISTITO_DIMISSIONE_ESITO_TRATTAMENTO = "10";
    @Mock
    ...
    RegolaEmurPSMesePresenzaCondizionataCampo regola;
    ...
    void init(){
        ...
        regola = spy(new RegolaEmurPSMesePresenzaCondizionataCampo("TEST", "TEST", "TEST", parametriTest));
    }

    @Test
    void costruttoreVuoto() {
        RegolaEmurPSMesePresenzaCondizionataCampo regola = new RegolaEmurPSMesePresenzaCondizionataCampo();
        assertTrue(regola instanceof RegolaEmurPSMesePresenzaCondizionataCampo);
    }
    ...
    void validaKO() throws InvocationTargetException, IllegalAccessException, NoSuchMethodException, MalformedRegistryException, RegistryNotFoundException {
        ...
        Mockito.when(recordMockito.getCampo("assistitoDatiAnagraficiEtaNascitaAnno")).thenReturn("1978");
        Mockito.when(recordMockito.getCampo("campoMeseNascita")).thenReturn("05");
        ...
        List<Esito> result = regola.valida("campoMeseNascita", recordMockito);
        for (Esito e : result) {
    ...
    void validaKO2() throws InvocationTargetException, IllegalAccessException, NoSuchMethodException, MalformedRegistryException, RegistryNotFoundException {
        ...
        Mockito.when(recordMockito.getCampo("assistitoDatiAnagraficiEtaNascitaAnno")).thenReturn("1978");
        Mockito.when(recordMockito.getCampo("campoMeseNascita")).thenReturn("05");
        Mockito.when(recordMockito.getCampo("assistitoDimissioneEsitoTrattamento")).thenReturn(ASSISTITO_DIMISSIONE_ESITO_TRATTAMENTO);
