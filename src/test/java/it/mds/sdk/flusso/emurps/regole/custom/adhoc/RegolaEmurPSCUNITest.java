...
class RegolaEmurPSCUNITest {
    ...
    void init(){
        ...
        regola = spy(new RegolaEmurPSCUNI("TEST", "TEST", "TEST", parametriTest));
    }

    @Test
    void costruttoreVuoto() {
        RegolaEmurPSCUNI regola = new RegolaEmurPSCUNI();
        assertTrue(regola instanceof RegolaEmurPSCUNI);
    }
    ...
    void validaKO2() throws InvocationTargetException, IllegalAccessException, NoSuchMethodException, MalformedRegistryException, RegistryNotFoundException {
        ...
        Mockito.when(recordMockito.getCampo("assistitoCUNI")).thenReturn("12345");
        ...
        List<Esito> result = regola.valida("assistitoCUNI", recordMockito);
        for (Esito e : result) {
