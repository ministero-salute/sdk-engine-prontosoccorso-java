...
package it.mds.sdk.flusso.emurps.service;
...
import it.mds.sdk.flusso.emurps.parser.regole.ParserTracciatoImpl;
...
import it.mds.sdk.flusso.emurps.tracciato.TracciatoSplitterImpl;
...
@Slf4j
@Component("flussoPSService")
public class FlussoPSService {

	private ParserRegole parserRegole;
	private ParserTracciato parserTracciato;
	private ValidationEngine validationEngine;
	private TracciatoSplitter<RecordDtoPS> tracciatoSplitter;
	private ConnettoreMds connettoreMds;
	private final ConfigurazioneFlussoPS conf;

	private final String CATEGORIA_FLUSSI = "PS1";
	private final String NOME_FLUSSO_CONTATTO = "PS1";
...
//	private static final String PERIODO_RIFERIMENTO_SEPARATOR = "I";
	...
	private final String NOME_FILE_XSD_PS = "EMUR-PS.xsd";

	@Autowired
	public FlussoPSService(@Qualifier("parserRegolePS") final ParserRegole parserRegole,
			...
			@Qualifier("tracciatoSplitter") final TracciatoSplitter<RecordDtoPS> tracciatoSplitter,
			@Qualifier("connettoreMds") final ConnettoreMds connettoreMds,
			@Qualifier("configurazioneFlussoPS") ConfigurazioneFlussoPS conf) {
	...
	public void validazioneBlocchi(int dimensioneBlocco, String nomeFile, RegoleFlusso regoleFlusso, String idRun,
		...
		// flusso
		FlsProSoc flsProSoc = null;
		File file = getfileFromPath(conf.getFlusso().getPercorso() + nomeFile);
		String nomeFileXml = conf.getXmlOutput().getPercorso() + "SDK_PS1_PS1_" + periodoRiferimento + "_" + idRun
				+ ".xml";
		...
		do {
			...
			List<RecordDtoPS> recordConv = bloccoValidazione.getRecordList().stream()
					.map(k -> (RecordDtoPS) k).collect(Collectors.toList());
			...
			if (!recordConv.isEmpty()) {
				log.debug("Per il blocco {} di idRun {} presenti {} record da trasformare in xml", numeroBlocco, idRun,
						recordConv.size());
				flsProSoc = ((TracciatoSplitterImpl) tracciatoSplitter).creaFlsProSoc(recordConv,flsProSoc);
			} else {
			...
			if (isWriteOk) {
				...
				URL urlXsd = this.getClass().getClassLoader().getResource("EMUR-PS.xsd");
				...
				try {
					gestoreFile.scriviDtoFragment(flsProSoc, nomeFileXmlTmp, urlXsd);
		...
		if (accettati > 0) {
			...
			try {
				nomeFilePulito = validationEngine.puliziaFileEmurPs(nomeFileXmlTmp, nomeFileXml, XML_FLUSSO_TAG);
				boolean isFileXMLValidated = validationEngine.validateXML(nomeFilePulito, NOME_FILE_XSD_PS);
...
//			periodoRiferimento = periodoRiferimento + PERIODO_RIFERIMENTO_SEPARATOR;
	...
	public void inviaTracciatoMds(String idRun, String nomeFileXml, GestoreRunLog gestoreRunLog,
		...
		try {
			...
			} else if (resp.getListaEsitiUpload().getItem().stream().filter(i -> !MDS_RESP_OK.equals(i.getEsito()))
			...
			if (resp.getErrorCode() == null || resp.getErrorCode().isBlank()) {
				...
				List<String> listStringhe = listaEsitiResp.getItem().stream().map(UploadEsito::getIdUpload)
